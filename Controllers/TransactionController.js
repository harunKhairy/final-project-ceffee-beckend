...
module.exports = {
  ...
  //                 return res
  //                   .status(500)
  //                   .send({ message: "error get data oncart" });
  ...
  //                 return res
  //                   .status(500)
  //                   .send({ err8, message: "error get data oncart" });
  ...
  //             return res
  //               .status(500)
  //               .send({ message: "error create new transactiondetails" });
  //           const token = createJWTToken({ id: userid, username });
  //           sql = `   SELECT * 
  //                               FROM transactions
  //                               WHERE id=${userid} AND status='oncart'`; //get data yg udah ada
  ...
  //                   WHERE t.id = ${id} AND t.status='oncart' AND td.isdelete=0`;
  //   db.query(sql, (err, result) => {
  //     if (err)
  //       return res.status(500).send({ err, message: "error get cart data" });
  ...
  getDataCart: (req, res) => {
    ...
    var sql = `   SELECT p.name, p.image, p.price, p.id AS productid, td.qty, td.id AS transactiondetailsid, t.id AS transactionid
                    FROM transactions t
                        ...
                        JOIN products p
                        ON td.productid=p.id
                    WHERE t.userid = ${id} AND t.status='oncart' AND td.isdelete=0`;
                    // !....................
    db.query(sql, (err, result) => {
      if (err)
        return res.status(500).send({ err, message: "error get cart data" });
      // const token=createJWTToken({id:id,username})
      res.status(200).send({result, message: "berhasil render cart"});
    });
  },
  ...
  deleteCart: (req, res) => {
    const { id } = req.params;
    var sql = `   UPDATE transactiondetails SET isdelete=1  
                    WHERE id=${id}`;
    db.query(sql, (err, result) => {
  ...
  transactionhistory: (req, res) => {
    ...
    var sql = `  SELECT t.id as transactionid, t.userid, t.metode, t.status
                    FROM transactions t
                        JOIN transactiondetails td
                        ON t.id=td.transactionid
                    WHERE t.id = ${userid} AND td.isdelete=0
                    GROUP BY transactionid
                    ORDER BY transactionid DESC`;
    db.query(sql, (err, result) => {
      if (err)
        res.status(500).send({ err, message: "error get transaction history" });
      var arr = [];
      result.forEach((element) => {
        arr.push(
          queryAsync(`   SELECT td.id AS transactiondetailid, td.transactionid, td.qty, p.name, p.image, p.price
                                        FROM transactions t
                                        JOIN transactiondetails td
                                        ON t.id=td.transactionid
                                        JOIN products p
                                        ON td.productid=p.id
                                        WHERE t.id=${element.transactionid} AND td.isdelete=0;`)
        );
      });
      Promise.all(arr).then((result1) => {
  ...
  gettransactiondetail: (req, res) => {
    ...
    var sql = `    SELECT t.id as transactionid, p.name,p.image,p.price, p.id AS productid,td.qty,td.id AS transactiondetailsid, t.status, u.username, u.address, u.phonenumber, t.method, t.creditcard_number
                        FROM transactions t
                            ...
                            JOIN user u
                            ON u.id=t.id
                        WHERE transactionid = ${transactionid} AND td.isdelete=0`;
    db.query(sql, (err, result) => {
      if (err)
        res.status(500).send({ err, message: "error get transaction detail" });
      res.status(200).send(result);
    });
  },
  getalltransaction: (req, res) => {
    var sql = `  SELECT t.id as transactionid, t.userid, t.metode, t.status, u.username
                    FROM transactions t
                        JOIN transactiondetails td
                        ON t.id=td.transactionid
                        JOIN user u
                        ON t.id=u.id
                    WHERE status!='oncart' AND td.isdelete=0
                    GROUP BY transactionid 
                    ORDER BY transactionid DESC`;
    db.query(sql, (err, result) => {
      if (err)
        res.status(500).send({ err, message: "error get all transaction id" });
      var arr = [];
      result.forEach((element) => {
        arr.push(
          queryAsync(`   SELECT td.id AS transactiondetailid, td.transactionid, td.qty, p.name, p.image, p.price
